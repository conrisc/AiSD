    ...
    for (int y=0;y<n && !found;y++) {
      for (int x=0;x<3 && !found;x++) {
        ...
        if (line[y][x] == '.') {
          ...
          if (y+2<n && x-1>=0 && line[y+2][x-1]=='.') {beats++; sum += howManyPositionsBeats(y+2,x-1,n);}
          if (y-2>=0 && x+1<3 && line[y-2][x+1]=='.') {beats++; sum += howManyPositionsBeats(y-2,x+1,n);}
          if (y+2<n && x+1<3 && line[y+2][x+1]=='.') {beats++; sum += howManyPositionsBeats(y+2,x+1,n);}
          if (y-1>=0 && x+2<3 && line[y-1][x+2]=='.') {beats++; sum += howManyPositionsBeats(y-1,x+2,n);}
          if (y+1<n && x+2<3 && line[y+1][x+2]=='.') {beats++; sum += howManyPositionsBeats(y+1,x+2,n);}

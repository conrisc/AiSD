...
int main() {
  ...
  for (int z=0;z<d;z++) {
    ...
    if (findHeight(&T,1,0,0)!=height) yup = false;
    ...
    for (int lvl=height-1;lvl>=0 && yup;lvl--) {
      ...
      for (int j=0;j<2 && yup;j++) {
        int is = 1;
        names[j][0].name = "1";
        T[j][names[j][0].id].name = names[j][0].name;
        for (size_t i=1;i<names[j].size() && yup;i++) {
          if (names[j][i-1].name!=names[j][i].name) {
            is++;
            names[j][i].name = to_string(is);
          }
          else {
            names[j][i].name = names[j][i-1].name;
          }
